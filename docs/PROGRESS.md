...
**阶段**：前端包已完成并推送到 GitHub，下一步建立最小可行后端（MVP 数据服务）  
**技术栈**：前端 Vite + 原生 JS；后端待定（FastAPI + PostgreSQL + MinIO 推荐）  
**基础构建工具**：Vite（已验证构建、开发服务器、ESLint 通过）  
**最近更新**：2026-07-21 — 完成音游右→左流动、判定偏移设置、悬浮设置窗、README，成功推送到 GitHub；新增后端起步方案文档。
...
1. 确定后端服务边界与最小可行架构（见本文件新增第 12 节）。
2. 确认前端框架与技术栈。
3. 提供 DEMO 视频，用于验证视频-谱面对齐与 UI 效果。
4. 基于技术栈开始实现第一个可运行 Demo（视频播放 + 硬编码谱面 + 音高匹配）。
5. 按 `PROJECT.md` 第 12 节 P0 优先级推进路演闭环。
